...
void CRenderer::SetWorldMatrix(const Vector3& v, const float angle){
  m_matWorld = XMMatrixRotationZ(angle) * XMMatrixTranslation(v.x, v.y, v.z);
} //SetWorldMatrix
...
void CRenderer::SetProjectionMatrix(){
  m_matProj = XMMatrixPerspectiveFovLH((float)XM_PI/4.0f, (float)g_nScreenWidth/g_nScreenHeight, 2.0f, 10000.0f);//Luis-Mess
} //SetProjectionMatrix
